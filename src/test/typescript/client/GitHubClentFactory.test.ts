...
describe('GitHubClientFactory', () => {
    it('can create client for Soup 3', () => {
        ...
        const client = GitHubClientFactory.newClient('my-extension', 'my-domain', 'my-token');

        assert.instanceOf(client, GitHubClientImpl);
        assert.instanceOf(client.httpEngine, Soup3HttpEngine);
    });
    ...
    it('throws error if Soup version is not recogniezed', () => {
        ...
        assert.throws(
            () => GitHubClientFactory.newClient('my-extension', 'my-domain', 'my-token'),
            'Unsupported Soup version: 5.3.7'
        );
    });
});
